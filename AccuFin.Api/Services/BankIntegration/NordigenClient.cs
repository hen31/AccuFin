...
namespace AccuFin.Api.Services.BankIntegration
{
    public class NordigenClient
    {
        ...
        internal async Task<BankLinkAuthorizationModel> GenerateBankLinkAsyc(string returnUrl, string institution)
        {
            ...
            HttpRequestMessage httpRequest = await GetAutherizedHttpRequest("https://ob.nordigen.com/api/v2/requisitions/");
            httpRequest.Method = HttpMethod.Post;

            string referenceId = Guid.NewGuid().ToString();
            string jsonContent = JsonConvert.SerializeObject(new { redirect = returnUrl, institution_id = institution, reference = referenceId });
        ...
        internal async Task<ICollection<BankModel>> GetBanksAsync()
        {
            ...
            return await result.Content.ReadFromJsonAsync<List<BankModel>>();
        }
